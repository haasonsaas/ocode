...
from ocode_python.tools.shell_tools_migration import (
...
async def demo_migration_tool():
    ...
    result = await tool.execute(
        command="echo 'Works with legacy interface!'",
        capture_output=True
    )
    print(f"Output: {result.output}")
    print(f"Tool name: {tool.definition.name}")  # Will be 'shell_command' for compatibility
