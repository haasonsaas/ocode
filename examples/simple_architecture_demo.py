...
from ocode_python.core.stream_processor import StreamProcessor, Operation
...
async def demo_all_together():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            ls_result = await orchestrator.get_task_result(high_priority_task, timeout=5)
            read_result = await orchestrator.get_task_result(normal_task, timeout=5)
            
            if ls_result and ls_result.success:
                file_count = len([line for line in ls_result.output.split('\n') if line.strip()])
