"""
Semantic context selection using embeddings and intelligent relevance scoring.

This module implements advanced semantic analysis for better context selection,
including embedding-based similarity, dynamic context expansion, and dependency following.
"""
...
from .context_manager import ContextManager, ProjectContext, FileInfo
...
class SemanticContextBuilder:
    ...
    def __init__(self, context_manager: ContextManager, cache_dir: Optional[Path] = None):
    ...
    async def _compute_semantic_scores(self, query: str, semantic_files: List[SemanticFile]) -> None:
    ...
    async def _process_embedding_batch(
        ...
        for semantic_file in batch:
            content_hash = hashlib.md5(semantic_file.content.encode()).hexdigest()
            ...
            try:
                ...
                for i, (semantic_file, content_hash) in enumerate(files_to_encode):
                    ...
                    self.embedding_cache.store_embedding(
                        str(semantic_file.path), content_hash, embedding, self.model_version
                    )
    ...
    async def _compute_keyword_scores(self, query: str, semantic_files: List[SemanticFile]) -> None:
    ...
    async def _compute_dependency_scores(self, semantic_files: List[SemanticFile]) -> None:
        ...
        for semantic_file in semantic_files:
            ...
            file_suffix = semantic_file.path.suffix if hasattr(semantic_file.path, 'suffix') else Path(semantic_file.path).suffix
            ...
            for imp in imports:
                ...
                for other_file in semantic_files:
                    if (imp in str(other_file.path) or 
                        other_file.path.stem == imp or
                        imp in other_file.content):
        ...
        for semantic_file in semantic_files:
            ...
            semantic_file.dependency_score = (
                (dependents / max_dependents) * 0.7 +
                (min(dependencies, 10) / 10) * 0.3
            )
    ...
    async def _compute_frequency_scores(self, semantic_files: List[SemanticFile]) -> None:
        ...
        for semantic_file in semantic_files:
            ...
            if semantic_file.path.name in ['main.py', 'index.js', 'app.py', '__init__.py']:
                score += 0.5
        ...
        for rule in self.expansion_rules:
            if re.search(rule.trigger_pattern, query, re.IGNORECASE):
                ...
                for additional_file in additional_files[:rule.max_expansion]:
                    ...
                    if not any(ef.path == additional_file.path for ef in expanded_files):
    ...
    async def _expand_by_rule(
        ...
        if rule.expansion_type == "imports":
            ...
            for semantic_file in selected_files:
                imports = self._extract_imports(semantic_file.content, semantic_file.path.suffix)
                ...
                for imp in imports:
                    for file_path, content in all_files.items():
                        ...
                        if isinstance(file_path, str):
                            file_path = Path(file_path)
                        if (imp in str(file_path) or 
                            file_path.stem == imp):
                            additional_file = SemanticFile(
                                path=file_path,
                                content=content,
                                similarity_score=0.5,  # Medium relevance
                                dependency_score=0.8,  # High dependency relevance
                                frequency_score=0.3
                            )
            ...
            for semantic_file in selected_files:
                ...
                for file_path, content in all_files.items():
                    ...
                    if isinstance(file_path, str):
                        file_path = Path(file_path)
                    if ('test' in file_path.name.lower() and 
                        stem in file_path.name.lower()):
                        additional_file = SemanticFile(
                            path=file_path,
                            content=content,
    ...
    def get_context_breadcrumbs(self, selected_files: List[SemanticFile]) -> List[str]:
        ...
        for semantic_file in selected_files:
            breadcrumb = f"{semantic_file.path.name} (score: {semantic_file.final_score:.2f}"
...
class DynamicContextManager:
    ...
    async def build_dynamic_context(
        ...
        semantic_files = await self.semantic_builder.build_semantic_context(
            query, base_context.files, max_files=int(initial_max_files * expansion_factor)
        )
        ...
        for semantic_file in semantic_files:
            ...
            if semantic_file.path in base_context.file_info:
                enhanced_file_info[semantic_file.path] = base_context.file_info[semantic_file.path]
