...
This module implements the stream processing architecture that separates reads from writes,
provides real-time streaming of intermediate results, and enables intelligent context batching.
"""
...
class ContextBatcher:
    ...
    def _group_files_by_characteristics(self, files: List[Path]) -> Dict[str, List[Path]]:
...
class StreamProcessor:
    ...
    def _organize_into_phases(self, operations: List[Operation]) -> List[PipelinePhase]:
        ...
        if analyze_ops:
            phases.append(PipelinePhase(
                phase_name="analyze",
                operations=sorted(analyze_ops, key=lambda x: x.priority, reverse=True),
                can_parallelize=True,
        ...
        if write_ops:
            phases.append(PipelinePhase(
                phase_name="write",
                operations=sorted(write_ops, key=lambda x: x.priority, reverse=True),
                can_parallelize=False,  # Writes must be sequential to avoid conflicts
                depends_on=["read", "analyze"] if (read_ops or analyze_ops) else []
            ))
    ...
    async def _execute_parallel_operations(
        ...
        tasks = [
            asyncio.create_task(execute_with_semaphore(op)) 
    ...
    async def stream_analysis(self, read_results: List[OperationResult]) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        for result in read_results:
            if result.success and result.result.output:
                ...
                analysis = {
                    ...
                    'content_preview': result.result.output[:200] + "..." if len(result.result.output) > 200 else result.result.output,
    ...
    async def batch_reads(self, read_operations: List[Operation]) -> List[OperationResult]:
        ...
        if file_operations:
            ...
            for batch in batches:
                batch_operations = [
                    ...
                    if Path(op.arguments['path']) in batch
                ]
    ...
    async def batch_writes(self, write_operations: List[Operation]) -> List[OperationResult]:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'successful_operations': sum(1 for r in self.operation_results.values() if r.success),
            'failed_operations': sum(1 for r in self.operation_results.values() if not r.success),
        }
