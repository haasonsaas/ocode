...
class TestEmbeddingCache:
    ...
    def test_embedding_storage_and_retrieval(self, embedding_cache):
        ...
        # Store embedding
        embedding_cache.store_embedding(file_path, content_hash, embedding, model_version)
        
        # Retrieve embedding
        retrieved = embedding_cache.get_embedding(file_path, content_hash, model_version)
    ...
    def test_memory_cache_limit(self, embedding_cache):
        ...
        for i in range(5):
            ...
            embedding_cache.store_embedding(f"file_{i}", f"hash_{i}", embedding, "model")
...
"""
    ...
    async def test_keyword_based_scoring(self, semantic_builder, sample_files):
        ...
        other_scores = [sf.similarity_score for sf in semantic_files if sf != config_file]
    ...
    async def test_frequency_scoring(self, semantic_builder, sample_files):
        ...
        assert main_file.frequency_score >= np.mean([sf.frequency_score for sf in other_files])
    ...
    async def test_context_expansion_tests(self, semantic_builder, sample_files):
        ...
        main_file = SemanticFile(
            ...
            content=sample_files[next(p for p in sample_files.keys() if p.name == "main.py")],
            final_score=1.0
        )
    ...
    async def test_semantic_scoring_with_embeddings(self, semantic_builder, sample_files):
...
class TestDynamicContextManager:
    ...
    def mock_base_context_manager(self):
        ...
        context_manager.scan_project = AsyncMock(return_value=[
            Path("/test/file1.py"), Path("/test/file2.py"), Path("/test/file3.py")
        ])
    ...
    async def test_dynamic_context_building(self, dynamic_context_manager):
        ...
        with patch.object(dynamic_context_manager.semantic_builder, 'build_semantic_context') as mock_build:
            mock_semantic_files = [
                SemanticFile(
                    path=Path("/test/file1.py"),
                    content="content1",
        ...
        for test_case in test_cases:
            ...
            for expected_file in test_case["expected_files"]:
                assert any(expected_file in name for name in selected_names), \
                    f"Expected {expected_file} in results for query '{test_case['query']}', got {selected_names}"
...
"""
        ...
        assert any("core.py" in name for name in expanded_names), \
