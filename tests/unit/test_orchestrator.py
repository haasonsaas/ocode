...
class TestConcurrentToolExecutor:
    ...
    @pytest.mark.asyncio
    async def test_independent_task_grouping(self, concurrent_executor, sample_tasks):
        """Test grouping of independent tasks."""
        groups = concurrent_executor._group_independent_tasks(sample_tasks)
        ...
        if len(file_tasks) > 1:
            ...
            resources1 = concurrent_executor._get_task_resources(file_tasks[0])
            resources2 = concurrent_executor._get_task_resources(file_tasks[1])
            assert resources1 != resources2
    
    @pytest.mark.asyncio
    async def test_parallel_execution(self, concurrent_executor, mock_tool_registry, sample_tasks):
    ...
    @pytest.mark.asyncio
    async def test_resource_conflict_detection(self, concurrent_executor):
        ...
        groups = concurrent_executor._group_independent_tasks(conflicting_tasks)
...
class TestAdvancedOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_task_submission_and_execution(self, orchestrator, mock_tool_registry):
    ...
    async def test_dependency_resolution(self, orchestrator, mock_tool_registry):
        ...
        try:
            ...
            # Manually set the prerequisite task ID (in real usage, this would be managed)
            ...
            dependent_result = await orchestrator.get_task_result(dependent_id, timeout=5.0)
    ...
    async def test_file_operation_workflow(self, real_tool_registry, tmp_path):
        ...
        try:
            ...
            tasks = [
                ...
                ("file_write", {"path": str(test_file), "content": "modified content"}, Priority.NORMAL),
            ...
            file_write_effects = [
                e for e in orchestrator.side_effect_broker.effects
